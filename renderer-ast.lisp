...
(defun render-ast-to-html (node)
  (cond
   ((null node) (format *error-output* "Got a NIL error. Report to Issues on Github, please") "")
   ...
   ((eq (ast-node-type node) :raw-html)
     (if (null (ast-node-content node))
         (progn
          (format *error-output* "Got a NIL error. Report to Issues on Github, please~%")
          "")
         (format nil "~A" (ast-node-content node))))

   ((eq (ast-node-type node) :heading)
     (if (null (ast-node-content node)) (progn (format *error-output* "Got a NIL error. Report to Issues on Github, please~%") "")
         (let ((parsed-inline-text (inline-walker (ast-node-content node))))
           (format nil "<h~D id=\"~A\">~A</h~D>"
             (ast-node-level node)
